...
Combined plot for all yeast effecting bacterial growth. 16 different isolates of yeast are grouped under 4 different classes of yeast.

```{r Plot for EMM_B44}
#read data
B44 <- read.csv("PeaceAssay/PeaceAssayData/2024-08-09_PeaceAssay_B44.csv", na.strings = "na") 
...
combined_plotB44 <- ggarrange(plotlist = plot_listB44, ncol = 2, nrow = 2, common.legend = FALSE)
...
```{r Stats for EMM_B44}
#filter data to remove 1st day data since the first data is taken as a base to measure the increase in colony size to rule out the variability that is caused by the drop inoculation. So, initially the increase in the colony diameter for 1st data is "0" that violates the assumption of normality, thus we remove that from analysis. This would be similar for all the bacterial isolates.
...
Days after inoculation (DAI) as a factor is always significantly impacting the growth. However, sometimes DAI is significant under interaction with other factor like Yeast and distance to the yeast as a factor. Also, our plot will represent data for Day 6 thus we want comparison for other variables at Day 6 to present it into the plot later on.
...
for (i in seq_along(day)) {
  ...
  resultsB44_Day[[i]] <- lme(increase ~ distance_to_yeast * Yeast,
                            ...
                            na.action = na.omit)
  cat("\n\n=== Day:", day[i], "===\n")
    model_summary_B44[[i]] <- summary(resultsB44_Day[[i]])
  anova_table_B44[[i]] <- anova(resultsB44_Day[[i]])
...
```{r Plot for EMM_B30}
#read data
B30 <- read.csv("PeaceAssay/PeaceAssayData/2024-07-21_PeaceAssay_B30.csv", na.strings = "na") 
...
```{r Plot for EMM_B5}
#read data
B5 <- read.csv("PeaceAssay/PeaceAssayData/2024-08-09_PeaceAssay_B5.csv", na.strings = "na") 
...
```{r Plot for AL65}
#read data
AL65 <- read.csv("PeaceAssay/PeaceAssayData/2024-08-09_PeaceAssay_AL65.csv", na.strings = "na") 
#load cbb color palette
cbbPalette <- c("#000000", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")
str(AL65)
#round off of the distance to yeast so that it is visibly a categorical variable
...
```{r Plot for EMM_B17}
#read data
B17 <- read.csv("PeaceAssay/PeaceAssayData/2024-07-17_PeaceAssay_B17.csv", na.strings = "na") 
...
```{r Plot for EMM_B52}
#read data
B52 <- read.csv("PeaceAssay/PeaceAssayData/MergedB52.csv", na.strings = "na") 
#load cbb color palette
cbbPalette <- c("#000000", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")
str(B52)
#round off of the distance to yeast so that it is visibly a categorical variable
...
```
 ...
 All significant letters will be assigned after the data is exported into pdf format for specific annotation on the plot.
